...
```r
library(BiocManager)
BiocManager::install(c("sesame", limna)) # use this to install packages
...
annot <- read.csv("data/normalized/90ArraySamples_simpleAnnot.csv")
annot$Sample_Group <- annot$SID
full_metadata <- merge(metadata, annot[, c("Sample_Group", "Treatment")], by="Sample_Group")

# Reformulate treatment variable
full_metadata$Treatment[full_metadata$Treatment == "CD+DFP"] <- "Treated"

# Discard beta values of samples dropped of full metadata
filtered_betas <- original_betas[, colnames(original_betas) %in% full_metadata$Sample_Group]
 
# Create design matrix
group <- factor(full_metadata$Treatment)
...
fit <- lmFit(filtered_betas, design)
...
# Need to pickup from here
# Get all results
results <- topTable(fit2, num = Inf, coef = 1)

# Add delta beta (biological effect size)
...
sig_dmps <- results[results$adj.P.Val < 0.05 & 
                    abs(results$deltaBeta) > 0.2, ]
