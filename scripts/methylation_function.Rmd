x# DNA Methylation analyses with Illumina Probes - Pipeline 2
...
final_dmps$strand <- annot[rownames(final_dmps), "strand"]
...
write.csv(genes_entrez, "results/list_genes_methylation.csv", row.names=FALSE)
...
### Extract genes from enriched KEGG and reperform Gene Ontology enrichment

```r
# Acquire genes
py_require(c("requests"))
source_python("codes/pathway2genes.py")

new_gene_list <- py$new_list

# Proceed to GO analysis

new_bp_geo <- geo(new_gene_list, "BP")
new_cc_geo <- geo(new_gene_list, "CC")
new_mf_geo <- geo(new_gene_list, "MF")

# Plot results

plot_go <- function(go, ont_type){
    # Draw barplot of results
    print(barplot(go))
    dev.copy(pdf, glue("results/barplot_go_{ont_type}_results.pdf"), width=15, height=10)
    dev.off()
    # Draw dotplot of results
    print(dotplot(go))
    dev.copy(pdf, glue("results/dotplot_go_{ont_type}_results.pdf"), width=15, height=10)
    dev.off()
}

plot_go(new_bp_geo, "BP")
plot_go(new_cc_geo, "CC")
plot_go(new_mf_geo, "MF")

# Extract list of genes and GO and save results

extract_gene_go <- function(go, annot_type){
    final_genes_go <- data.frame()
    go_data <- go[]

    for (i in seq_len(nrow(go_data))) {
        genes <- strsplit(go_data$geneID[i], "/")[[1]]
  
        for (gene in genes) {
            temp <- data.frame(
                gene_symbol = gene,
                go_id = go_data$ID[i],
                go_description = go_data$Description[i],
                p_adjusted = go_data$p.adjust[i],
                stringsAsFactors = FALSE
            )
            temp <- temp[order(temp$p_adjusted),]
            final_genes_go <- rbind(final_genes_go, temp)
        }
    }
    final_genes_go <- unique(final_genes_go)
    write.csv(final_genes_go, glue("results/list_genes_go_{annot_type}.csv"), row.names=FALSE)
}

extract_gene_go(new_bp_geo, "BP")
extract_gene_go(new_cc_geo, "CC")
extract_gene_go(new_mf_geo, "MF")

```


### Perform KEGG Pathway Analysis with AI results

```r
# Read genes from file
genes_ai <- readLines("results/AI2gene_list.txt")

# Extract gene IDs
gene_list_temp <- bitr(genes_ai,
                     fromType = "SYMBOL",
                     toType = "ENTREZID",
                     OrgDb = org.Mm.eg.db)

gene_list_ai <- gene_list_temp$ENTREZID

# Proceed to analysis
kegg_ai <- enrichKEGG(gene = gene_list_ai,
                   organism = 'mmu',
                   pvalueCutoff = 0.05)

# Draw barplot of results
barplot(kegg)
dev.copy(pdf, "results/barplot_kegg_AI_results.pdf", width=10)
dev.off()

# Draw dotplot of results
dotplot(kegg)
dev.copy(pdf, "results/dotplot_kegg_AI_results.pdf", width=10)
dev.off()


# Extract table
final_genes_kegg_ai <- data.frame()

kegg_data_ai <- kegg_ai[]

for (i in seq_len(nrow(kegg_data_ai))) {
  genes <- strsplit(kegg_data_ai$geneID[i], "/")[[1]]
  genes <- as.integer(genes)
  
  matches <- gene_list_temp$ENTREZID %in% genes
  
  if (any(matches)) {
    temp <- data.frame(
        gene_entrez_id = gene_list_temp$ENTREZID[matches],
        gene_symbol = gene_list_temp$SYMBOL[matches],
        pathway_id = kegg_data_ai$ID[i],
        pathway_description = kegg_data_ai$Description[i],
        p_adjusted = kegg_data_ai$p.adjust[i],
        stringsAsFactors = FALSE
    )
    temp <- temp[order(temp$p_adjusted),]
    final_genes_kegg_ai <- rbind(final_genes_kegg_ai, temp)
  }
}

final_genes_kegg_ai <- unique(final_genes_kegg_ai)
write.csv(final_genes_kegg_ai, "results/list_genes_kegg_AI.csv", row.names=FALSE)
```


### Perform Gene Ontology Enrichment with AI results

```r
# Proceed to analysis using previously defined functions
bp_geo_ai <- geo(gene_list_ai, "BP")
cc_geo_ai <- geo(gene_list_ai, "CC")
mf_geo_ai <- geo(gene_list_ai, "MF")

plot_go(bp_geo_ai, "BP_AI")
plot_go(cc_geo_ai, "CC_AI")
plot_go(mf_geo_ai, "MF_AI")

# Extract tables
extract_gene_go(bp_geo_ai, "BP_AI")
extract_gene_go(cc_geo_ai, "CC_AI")
extract_gene_go(mf_geo_ai, "MF_AI")
```
...
# Save all methylation phenotypes

save_methyl <- function(methyl_pheno, annot_type){
    colnames(methyl_pheno) <- metadata$Strain[match(colnames(methyl_pheno), metadata$ExtBasename)]

    trans_methyl_pheno <- t(methyl_pheno)

    write.table(trans_methyl_pheno, glue("~/{annot_type}_phenofile.txt"), row.names=FALSE, col.names=FALSE, sep=", ", quote=FALSE) # without strains

    #write.table(trans_methyl_pheno, glue("~/{annot_type}_phenofile_strains.txt"), row.names=TRUE, col.names=TRUE, sep=", ", quote=FALSE) # with strains and phenotypes
}

save_methyl(beta, "beta")
save_methyl(log2(beta), "log2beta")
save_methyl(qsmoothData(qsmooth(object=beta, group_factor=groups)), "quantile_beta")

save_methyl(M, "M")


# Save only methylation phenotypes for DMPs

save_rmethyl <- function(methyl_pheno, annot_type){
    keep <- rownames(methyl_pheno) %in% rownames(final_dmps)
    inter_methyl_pheno <- methyl_pheno[keep, , drop=FALSE]

    colnames(inter_methyl_pheno) <- metadata$Strain[match(colnames(inter_methyl_pheno), metadata$ExtBasename)]

    tinter_methyl_pheno <- t(inter_methyl_pheno)

    write.table(tinter_methyl_pheno, file=glue("~/interest_{annot_type}_phenofile.txt"), row.names=FALSE, col.names=FALSE, sep=", ", quote=FALSE) # without strains

    #write.table(tinter_methyl_pheno, file=glue("~/interest_{annot_type}_phenofile_strains.txt"), row.names=TRUE, col.names=TRUE, sep=", ", quote=FALSE) # with strains and phenotypes
}

save_rmethyl(beta, "beta")
save_rmethyl(log2(beta), "log2beta")
save_rmethyl(qsmoothData(qsmooth(object=beta, group_factor=groups)), "quantile_beta")

save_rmethyl(M, "M")


# Save methylation phenotypes of interest + neighbors

save_mmethyl <- function(methyl_pheno, annot_type){
    final_results <- data.frame()
    temp_results <- subset(results, chr %in% unique(final_dmps$chr))
    
    for (new_pos in final_dmps$pos){
        temp <- subset(temp_results, abs(pos-new_pos)<10000)
        final_results <- rbind(final_results, temp)
    }
    final_results <- unique(final_results)

    keep <- rownames(methyl_pheno) %in% rownames(final_results)
    final_pheno <- methyl_pheno[keep, , drop=FALSE]

    colnames(final_pheno) <- metadata$Strain[match(colnames(final_pheno), metadata$ExtBasename)]
    
    trans_final_pheno <- t(final_pheno)

    write.table(trans_final_pheno, glue("~/more_{annot_type}_phenofile.txt"), row.names=FALSE, col.names=FALSE, sep=", ", quote=FALSE) # without strains

    #write.table(trans_final_pheno, glue("~/more_{annot_type}_phenofile_strains.txt"), row.names=TRUE, col.names=TRUE, sep=", ", quote=FALSE) # with strains and phenotypes
}

save_mmethyl(beta, "beta")
save_mmethyl(log2(beta), "log2beta")
save_mmethyl(qsmoothData(qsmooth(object=beta, group_factor=groups)), "quantile_beta")
